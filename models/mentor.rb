...
class Mentor < Sequel::Model
  ...
  def getApplications()
    pendingMatches = PendingMatch.where(Sequel.ilike(:mentor_id, self.id)).all
    for x in 0...pendingMatches.length do
      pendingMatches[x] = [Mentee.where(Sequel.ilike(:id, pendingMatches[x].getMenteeId)).first.username,
      Mentee.where(Sequel.ilike(:id, pendingMatches[x].getMenteeId)).first.course,
      Mentee.where(Sequel.ilike(:id, pendingMatches[x].getMenteeId)).first.date_of_birth,
      Mentee.where(Sequel.ilike(:id, pendingMatches[x].getMenteeId)).first.bio,
      Mentee.where(Sequel.ilike(:id, pendingMatches[x].getMenteeId)).first.id]
    end
